...
pool.getConnection((err, connection) => {
  if (err) {
    ...
    connection.execute(createTestTable, (err) => {
      console.log('database.js: Test table creation successful');

      if (err) {
        console.error('database.js: Error creating a test table :', err);
        process.exit();
      } else {
        connection.execute('drop table if exists test');
        connection.release();
        console.log('database.js: All set and ready');
      }
    });
  }
});
