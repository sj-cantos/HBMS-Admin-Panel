        ...
        pool.execute(
          ...
          (err, roomtypes_res) => {
            if (err) {
              ...
              res
                .status(500)
                .json({ status: 500, message: "Server Error after query" });
              return;
            } else {
              const room_id = roomtypes_res[0].id;
              pool.execute(
                `UPDATE hotel_bookings SET guest_name = ?,email = ?, room_type = ?, booking_date = ?, check_in_date = ?, 
  ...
  pool.execute(
    ...
    (err, result) => {
      if (err) {
        ...
        res
          .status(200)
          .json({ status: 200, message: "Deleted booking data successfully" });
      }
    }
  );
});
