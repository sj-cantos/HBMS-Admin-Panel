      ...
      (err, result) => {
        if (err) {
          ...
          return callback(null, false, {
            message: "That user admin does not exists in the database",
          }); // for dev only
          // return callback(null, false, { message: 'Incorrect username or password'}); // for release
        } else if (result.length > 1) {
          return callback(null, false, {
            message:
              "That was not supposed to happen, the database table might be broken",
          }); // for dev only
        }

        crypto.pbkdf2(
          Buffer.from(password, "utf-8").toString(),
          Buffer.from(result[0].salt, "base64"),
          200000,
          64,
          "sha512",
          function (err, hashedPassword) {
            if (err) {
              ...
              !crypto.timingSafeEqual(
                hashedPassword,
                Buffer.from(result[0].hash, "base64")
              )
            ) {
